...
class TranslationGamePage extends StatefulWidget {
  const TranslationGamePage({Key? key}) : super(key: key);

  @override
  State<TranslationGamePage> createState() => _TranslationGamePageState();
}

class _TranslationGamePageState extends State<TranslationGamePage> {
  ...
  void checkHighScoreAndSave() async {
    ...
    if (correctAnswerStreak > flag) {
      prefs.setInt(kHighScoreKeyTranslationGame, correctAnswerStreak);
    }
  }
  ...
  void getRandomData() {
    setState(() {
      ...
      answerOptions = _helper.getRandomEnglislhWords(data['english']);
      showingOptions = List.from(answerOptions);
      selectedAnswer.clear();
      answer = data['english'].trim().split(' ');
      answerResult = null;
    });
  }
